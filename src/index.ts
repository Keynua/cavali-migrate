...
import { program } from "./_script";
import { EnvironmentConfig } from "./_types";
 ...
 */
function parseToSend(toSendStr?: string): (number | [number, number])[] | undefined {
  if (!toSendStr) return undefined;
 ...
 */
function buildConfigFromEnv(): EnvironmentConfig {
 ...
 */
function getEnvironment(): 'dev' | 'stg' | 'prod' {
...
program(config, environment).then(console.log);
