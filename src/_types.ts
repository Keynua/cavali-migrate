...
export enum DocumentType {
  ...
  CARNET_EXTRANJERIA = 3, // Foreigner ID Card
}

/**
 * Condition for signature-only promissory notes
 */
export enum ConditionJustSign {
  SI = 1, // Yes - signature only
  NO = 2, // No - requires additional conditions
}
...
export interface ParseError {
  row: number; // Row number where the error occurred (1-indexed)
...
export interface KeyConfig {
  prefix: string; // Client identifier prefix for file naming
...
export interface EnvironmentConfig {
  file: string; // Path to the Excel file to process
  batch?: number; // Starting batch number (defaults to 1)
  maxBatch?: number; // Maximum batch number to process (defaults to all batches)
  batchSize: number; // Number of records per batch
  prefix: string; // Client identifier prefix
  authorization: string; // Authorization token
  apikey: string; // API key
  toSend?: (number | [number, number])[]; // Optional filter: specific rows or ranges to send
}
...
export interface ProgramResult {
  batchId: number; // Unique identifier for this execution batch
}
...
export interface PayloadsResult {
  errors: ParseError[]; // List of parsing errors encountered
  payloads: Payload[]; // Successfully parsed payloads
}
